...
import com.github.didahdx.githubapp.data.local.dao.FollowerDao
...
class FollowersRemoteMediator constructor(
    ...
    override suspend fun load(
        ...
        try {
            val repos = gitHubApiService.getFollowerUsersList(query, page, state.config.pageSize)

            val endOfPaginationReached = repos.isEmpty()
            githubDatabase.withTransaction {
                // clear all tables in the database
                if (loadType == LoadType.REFRESH) {
                    remoteKeysDao.clearRemoteKeys(FOLLOWERS)
                    followerDao.clearUsers()
                }
                ...
                followerDao.insertAll(repos.map { it.mapToUserEntityForFollowers(query) })
            }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { repoId ->
                remoteKeysDao.remoteKeysRepoId(repoId.toLong(), FOLLOWERS)
            }
        }
    }


    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, FollowersEntity>): RemoteKeys? {
        // Get the first page that was retrieved, that contained items.
        // From that first page, get the first item
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { repo ->
                // Get the remote keys of the first items retrieved
                remoteKeysDao.remoteKeysRepoId(repo.id.toLong(), FOLLOWERS)
            }
    }

    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, FollowersEntity>): RemoteKeys? {
